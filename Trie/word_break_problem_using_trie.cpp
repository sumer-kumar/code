...
int isValid(tnode* root,string sent);
...
int isWordBreak(string sent,string B[],int n);
...
int main() {
    ...
    cout<<isWordBreak("ilike",dictionary,n)<<endl;
...
int isValid(tnode* root,string sent)
{
	...
	{
		if(search(root,sent.substr(0,i)) && isValid(root,sent.substr(i,size-i)))
			return 1;
	}
	return 0;
}
